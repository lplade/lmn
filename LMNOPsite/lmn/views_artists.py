...
def venues_for_artist(request, artist_pk):   # pk = artist_pk
    ...
    shows = Show.objects.filter(artist=artist_pk).order_by(
        'show_date').reverse()  # most recent first
    artist = Artist.objects.get(pk=artist_pk)

    return render(request, 'lmn/venues/venue_list_for_artist.html',
                  {'artist': artist, 'shows': shows})
...
def artist_detail(request, artist_pk):
    ...
    return render(request, 'lmn/artists/artist_detail.html',
                  {'artist': artist})
